...
export const removeCartItem = async (req: Request, res: Response) => {
  const { cartId, lineIds } = req.body as {
    cartId: string
    lineIds: string[]
  }
  ...
  if (!cartId || !lineIds || !Array.isArray(lineIds)) {
    return res.status(400).json({
      error: "Missing or invalid cartId or lineIds",
    })
  }
...
`
    ...
    while (more) {
      const variables: GetCartVariables = { input: cartId, cursor: cursor }
...
`
    ...
    if (!data.cart) return res.status(200).json({ message: "No cart found" })
